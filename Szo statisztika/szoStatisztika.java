...
Azzal nagy büszkén odaadta neki a lyukas krajcárt, amit a vándorútján szerzett.
		...
		String[] szavak = mese.split(" ");
		Map<String,Integer> szoStat=new HashMap<>();
		for (String szo : szavak) {
			if(szo.length()>1) {
				if(szoStat.containsKey(szo)) {
					Integer ertek = szoStat.get(szo);
					ertek++;
					szoStat.put(szo, ertek);
				}else {
					szoStat.put(szo, 1);
				}
			}
		...
		for (Entry<String, Integer> stat : szoStat.entrySet()) {
			if(stat.getValue()>1) {
